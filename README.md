...
Systemd service and timer scripts are provided in `src/systemd`. Move them to `~/.config/systemd/user` and enable using `systemctl --user enable --now lima_ddns.timer`.
